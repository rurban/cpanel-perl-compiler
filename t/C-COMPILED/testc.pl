...
$file_to_test =~ m/0*(\d+?)\.t/
  or die("$file_to_test cannot be recognized as a testc test!");
my $testc_test = $1;
    ...
    foreach my $optimization (@optimizations) {
      TODO: SKIP: {
            ...
            unless ( -x $bin_file ) {
                ...
                skip(
                    "Can't test further due to failure to create a binary file.",
                    $to_skip
                );
            }
            ...
            unless ( $signal == 0 ) {
                note $out if ($out);
                skip(
                    "Test failures irrelevant if exits premature with $sig_name",
                    $to_skip
                );
            }
        }
